...
Thread::Finish ()
{
    ...
    #ifdef USER_PROGRAM
    waitingThreadExitCode = currentThread->getExitStatus();  // 获取退出状态
...
Thread::Join(int SpaceID) 
{
    ...
    currentThread->waitingProcessSpaceId = SpaceID; //设置等待的进程ID
    ...
    printf("inTerminatedList %d\n", inTerminatedList);
    // 如果要等待的进行尚未终止，不在终止队列中
    if(!inTerminatedList){
        printf("not in terminated list\n");
        this->waitingProcessSpaceId = SpaceID;
        printf("xxx");
