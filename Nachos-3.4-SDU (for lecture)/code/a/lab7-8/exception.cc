...
#include "system.h"
#include "syscall.h"
#include "openfile.h"   // new
#include "progtest.h"   // new
...
ExceptionHandler(ExceptionType which)
{
    ...
    if (which == SyscallException) {
        ...
        case SC_Join: {
            ...
            printf("start join thread %d\n", SpaceID);
            currentThread->Join(SpaceID);
            printf("end join thread %d\n", SpaceID);
            //返回 Joinee 的退出码 waitProcessExitCode
            machine->WriteRegister(2, currentThread->waitProcessExitCode);  // 返回 Joinee 的退出码
            AdvancePC();
            printf("CurrentThreadId: %d Name: %s, Joined thread %d, exit code is %d\n");
            break;
        }
        case SC_Yield:{
